...
class VAE(chainer.Chain):
    ...
    def __init__(self, n_in, n_latent, n_h):
        ...
        with self.init_scope():
            ...
            self.le2_mu = L.Linear(n_h, n_latent, initialW=xavier.Xavier(n_h, n_latent))
            self.le2_ln_var = L.Linear(n_h, n_latent, initialW=xavier.Xavier(n_h, n_latent))
            # decoder
            self.ld1 = L.Linear(n_latent, n_h, initialW=xavier.Xavier(n_latent, n_h))
            self.ld2 = L.Linear(n_h, n_in, initialW=xavier.Xavier(n_h, n_in))
